...
const baseURL = "http://172.17.18.62:443/";
...
var blueClusteredIcon	= L.icon({iconUrl:'Points/pointBlueClustered.png', iconSize:[24, 24],iconAnchor:[12, 12],popupAnchor:[0, 0]});
var greenClusteredIcon	= L.icon({iconUrl:'Points/pointGreenClustered.png', iconSize:[24, 24],iconAnchor:[12, 12],popupAnchor:[0, 0]});
var yellowClusteredIcon	= L.icon({iconUrl:'Points/pointYellowClustered.png', iconSize:[24, 24],iconAnchor:[12, 12],popupAnchor:[0, 0]});
var orangeClusteredIcon	= L.icon({iconUrl:'Points/pointOrangeClustered.png', iconSize:[24, 24],iconAnchor:[12, 12],popupAnchor:[0, 0]});
var redClusteredIcon	= L.icon({iconUrl:'Points/pointRedClustered.png', iconSize:[24, 24],iconAnchor:[12, 12],popupAnchor:[0, 0]});
/*
...
map.on('moveend', function(event) {   
	...
	if( oldCenter.distanceTo(map.getCenter()) > 2000 || map.getZoom() != oldZoom) {
		deleteData();
		catchData().catch(error => {
			console.log('Error!');
			console.error(error);
...
async function catchData(){
	const url = baseURL + "localValues?lat=" + map.getCenter().lat + "&lng=" + map.getCenter().lng + "&zoom=" + map.getZoom() + "&accuracy=" + 10;
	//console.log(url);
	...
	data.forEach(element => {
        ...
        if(element.clustered == true) {
            if (ppm < minPPM * 1.10) {
                usedIcon = blueClusteredIcon;
            } else if (ppm > maxPPM * 0.90) {
                usedIcon = redClusteredIcon;
            } else if (ppm >= avgPPM * 0.90 && ppm <= avgPPM * 1.10) {
                usedIcon = yellowClusteredIcon;
            } else if (ppm < avgPPM) {
                usedIcon = greenClusteredIcon;
            } else if (ppm > avgPPM) {
                usedIcon = orangeClusteredIcon;
            }
        } else {
            if (ppm < minPPM * 1.10) {
                usedIcon = blueIcon;
